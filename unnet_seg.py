...
# Load Patient Folder
INPUT_FOLDER = 'C:/Users/stephen_GAME/Google Drive/UB documents/Spring 2017/BE 400/kaggle_group/sample2_images/'  # Note, this is for sample images
...
lungVolume  = definitions.segment_lung_mask(pixels, False)
...
high_vals   = extVolume == 0
innerVals   = smVolume == 1
pixels[high_vals] = 0
pixels[innerVals] = 0
definitions.plot_ct_scan(pixels)
extVolume[smVolume == 1] = 0
definitions.plot_ct_scan(extVolume)

#filename = 'can_delete2.dat'
#fp = definitions.np.memmap(filename, dtype='int16', mode='w+', shape=(36175872,1400))


# for idx, patient in enumerate(patients[0]):
#     data        = definitions.load_scan(INPUT_FOLDER + patient)
#     pixels      = definitions.get_pixels_hu(data)
#     lungVolume  = definitions.segment_lung_mask(pixels, True)
#     extVolume   = definitions.ext_mask(lungVolume,3)
#     smVolume    = definitions.scipy.ndimage.morphology.binary_erosion(lungVolume,iterations=3)
#     high_vals   = extVolume == 0
#     pixels[high_vals] = 0
#     definitions.plot_2d(pixels)
